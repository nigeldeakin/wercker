...
func (cp *RunnerParams) RunDockerController(statusOnly bool) {
	...
	if statusOnly == true {
		...
		cp.Logger.Print("There are no runners active for %s", cp.Basename)
		return
	}
	...
	// Validate the group name. If omitted then default to the base name. When present
	// check for an org specification and disallow spps/workspaces parameters
	if cp.GroupName == "" {
		cp.GroupName = cp.Basename
	} else {
		if strings.IndexByte(cp.GroupName, '@') != -1 {
			if cp.AppNames != "" || cp.Workflows != "" {
				cp.Logger.Fatal("--apps or --workflows not allowed with groupname@organization")
			}
		}
	}
	...
	if !cp.NoWait {
		...
		cp.Logger.Info("Use the Wercker runner stop command with the same name to terminate the started runners.")
	}
}
